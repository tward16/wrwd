...
lvector.CartoDB = lvector.GeoJSONLayer.extend({
    ...
    _getFeatures: function() {
        ...
        a = a.length ? " " + a : "";
        a = "https://" + this.options.user + ".cartodb.com/api/v" + this.options.version + "/sql?q=" + encodeURIComponent("SELECT " + this.options.fields + " FROM " + this.options.table + (a.length ? " WHERE " + a : "")) + "&api_key=" + this.options.key + "&format=geojson&callback=" + this._globalPointer + "._processFeatures";
        this._makeJsonpRequest(a)
        }
    }
});
